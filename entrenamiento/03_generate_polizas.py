"""Genera la tabla de polizas sinteticas. Requiere: data/synthetic/asegurados.csv. Salida: data/synthetic/polizas.csv (~8,000 registros)"""
...
OUTPUT_PATH     = os.path.join(os.path.dirname(__file__), "..", "data", "synthetic", "polizas.csv")
...
# Parametros por ramo: prima, suma asegurada, deducible
PARAMETROS_RAMO = {
...
def generar_poliza(idx: int, id_asegurado: str, ciudad_asegurado: str) -> dict:
    ...
    if params["deducible"][1] == 0:
        deducible = 0.0
    else:
        deducible = round(random.uniform(*params["deducible"]), 2)

    # 10% de probabilidad de ciudad diferente a la del asegurado
...
def main():
    ...
    # Asegurados con mas polizas tienen mayor probabilidad de ser elegidos
    ...
    # Ruido realista en primas y suma asegurada
    np.random.seed(SEED)
    ...
    print(f"   Ramos: {df['ramo'].value_counts().to_dict()}")
    print(f"   Estados: {df['estado_poliza'].value_counts().to_dict()}")
    print(f"   Prima media: ${df['prima'].mean():,.2f}")
    print(f"   Suma asegurada media: ${df['suma_asegurada'].mean():,.2f}")
