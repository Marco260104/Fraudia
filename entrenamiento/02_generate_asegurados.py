"""Genera la tabla de asegurados sinteticos. Salida: data/synthetic/asegurados.csv (~6,000 registros)"""
...
SEED = 42
N_ASEGURADOS = 6_000
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "synthetic", "asegurados.csv")
...
def generar_asegurado(idx: int) -> dict:
    """Genera un asegurado sintetico con perfil de riesgo coherente."""
    ...
    ciudad = random.choices(
        CIUDADES,
        # Quito y Guayaquil concentran más asegurados
        weights=[25, 22, 10, 6, 5, 4, 4, 4, 5, 4, 2, 2, 3, 2, 2]
    )[0]
...
def main():
    ...
    print(f"   Perfiles: {df['perfil_riesgo'].value_counts().to_dict()}")
    print(f"   Segmentos: {df['segmento'].value_counts().to_dict()}")
    print(f"   Reclamos 12m media: {df['reclamos_12m'].mean():.2f}")
    print(f"   Score cliente media: {df['score_cliente'].mean():.1f}")
